...
public final class TopologicalSort {
    ...
    public static <T> List<T> topologicalSort(SimpleValueGraph<T, ?> graph, Comparator<? super T> comparator)
        ...
        while (!queue.isEmpty()) {
            ...
            for (final T successor : graph.successors(current)) {
                final int updated = degrees.compute(
                    successor,
                    (node, degree) -> Objects.requireNonNull(degree, () -> "Invalid degree present for " + node) - 1);
                if (updated == 0) {
                    queue.add(successor);
