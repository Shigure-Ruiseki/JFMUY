...
import ruiseki.jfmuy.gui.ingredients.GuiIngredientFast;
import ruiseki.jfmuy.gui.ingredients.GuiIngredientFastList;
...
public class ItemListOverlayInternal implements IShowsRecipeFocuses, IMouseHandler, IKeyable {
    ...
    public ItemListOverlayInternal(ItemListOverlay parent, IIngredientRegistry ingredientRegistry, GuiScreen guiScreen,
        ...
        searchField = new GuiTextFieldFilter(
            0,
            fontRenderer,
            searchFieldX,
            searchFieldY,
    ...
    public void updateLayout() {
        ImmutableList<IIngredientListElement> ingredientList = parent.getItemFilter()
            .getIngredientList();
    ...
    private void nextPage() {
        final int itemsCount = parent.getItemFilter()
            .size();
        if (itemsCount == 0) {
            firstItemIndex = 0;
            return;
    ...
    public void drawScreen(Minecraft minecraft, int mouseX, int mouseY) {
        ...
        if (!highlightedStacks.isEmpty()) {
            StackHelper helper = Internal.getHelpers()
                .getStackHelper();
    ...
    private int getPageCount() {
        final int itemCount = parent.getItemFilter()
            .size();
