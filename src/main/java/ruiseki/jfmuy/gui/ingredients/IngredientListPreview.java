 ...
 * {@link ruiseki.jfmuy.gui.TooltipRenderer#drawHoveringTextAndItems}.
 * <p>
...
public class IngredientListPreview {
     ...
     */
    @Nullable
    public static <T> IngredientListPreview create(List<T> ingredients, IIngredientHelper<T> ingredientHelper,
        IIngredientRenderer<T> ingredientRenderer, IModIdHelper modIdHelper) {
        ...
        for (T ingredient : ingredients) {
            ...
            IIngredientListElement<T> element = IngredientListElement
                .create(ingredient, ingredientHelper, ingredientRenderer, modIdHelper, 0);
            if (element != null) {
    ...
    private IngredientListPreview(List<IIngredientListElement<?>> elements) {
        ...
        this.scrollBar = new ScrollBar(
            ...
            0,
            GRID_HEIGHT,
            guiHelper.getScrollbarBackground(),
            guiHelper.getScrollbarMarker());
    ...
    public boolean dragScrollTo(int screenMouseY) {
        ...
        ScrollBar.ScrollResult result = scrollBar
            .dragTo(screenMouseY - origin.y, MAX_VISIBLE, hiddenCount(), scrollOffset);
        if (result.isHandled()) {
            setScrollOffset(result.getScrollOffsetY());
            return true;
        }
