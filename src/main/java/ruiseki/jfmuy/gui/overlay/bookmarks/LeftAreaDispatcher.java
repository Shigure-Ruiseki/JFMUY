...
public class LeftAreaDispatcher implements IShowsRecipeFocuses, IGhostIngredientDragSource, IPaged {
    ...
    public void drawScreen(Minecraft minecraft, int mouseX, int mouseY) {
        if (canShow && hasContent()) {
            contents.get(current)
                .drawScreen(minecraft, mouseX, mouseY);
            if (naviArea.height > 0) {
                navigation.draw(minecraft, mouseX, mouseY);
            }
        }
    }
    ...
    public void drawOnForeground(GuiContainer gui, int mouseX, int mouseY) {
        if (canShow && hasContent()) {
            contents.get(current)
                .drawOnForeground(gui, mouseX, mouseY);
        }
    }
    ...
    public void drawTooltips(Minecraft minecraft, int mouseX, int mouseY) {
        if (canShow && hasContent()) {
            contents.get(current)
                .drawTooltips(minecraft, mouseX, mouseY);
        }
    }
