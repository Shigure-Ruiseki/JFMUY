...
public class Textures {
    ...
    private final JFMUYTextureMap textureMap;
    ...
    public Textures(JFMUYTextureMap textureMap) {
        this.textureMap = textureMap;

        this.slot = registerGuiSprite("slot", 18, 18).slice(4, 4, 4, 4);
        this.tabSelected = registerGuiSprite("tab_selected", 24, 24);
        this.tabUnselected = registerGuiSprite("tab_unselected", 24, 24);
        this.buttonDisabled = registerGuiSprite("button_disabled", 20, 20).slice(2, 2, 2, 2);
        this.buttonEnabled = registerGuiSprite("button_enabled", 20, 20).slice(2, 2, 2, 2);
        this.buttonHighlight = registerGuiSprite("button_highlight", 20, 20).slice(2, 2, 2, 2);
        this.guiBackground = registerGuiSprite("gui_background", 64, 64).slice(16, 16, 16, 16);
        this.recipeBackground = registerGuiSprite("single_recipe_background", 64, 64).slice(16, 16, 16, 16);
        this.searchBackground = registerGuiSprite("search_background", 20, 20).slice(4, 4, 4, 4);
        this.catalystTab = registerGuiSprite("catalyst_tab", 28, 28).slice(8, 9, 8, 8);

        this.shapelessIcon = registerGuiSprite("icons/shapeless_icon", 36, 36).trim(1, 2, 1, 1);
    ...
    private TextureInfo registerGuiSprite(String name, int width, int height) {
        TextureAtlasSprite textureAtlasSprite = textureMap
            .registerSprite(new ResourceLocation(Reference.MOD_ID, "gui/" + name));
