    ...
    public JemiCategory(IRecipeCategory<?> category) {
        ...
        this.icon = (raw, x, y, delta) -> {
            ...
            if (icon != null) {
                ...
                List<EmiIngredient> workstations = EmiApi.getRecipeManager()
                    .getWorkstations(this);
                if (!workstations.isEmpty()) {
                    workstations.get(0)
                        .render(context.raw(), x, y, delta, EmiIngredient.RENDER_ICON);
                } else {
                    String title = category.getTitle();
                    context.drawCenteredTextWithShadow(
                        EmiPort.literal(title.substring(0, Math.min(2, title.length()))),
                        x + 8,
                        y + 2);
                }
            }
