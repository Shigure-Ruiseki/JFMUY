...
import ruiseki.jfmuy.input.GuiContainerWrapper;
import ruiseki.jfmuy.input.IClickedIngredient;
import ruiseki.jfmuy.plugins.jemi.impl.JemiIngredients;
import ruiseki.jfmuy.plugins.jemi.runtime.JemiDragDropHandler;
import ruiseki.jfmuy.plugins.jfmuy.info.IngredientInfoRecipe;
import ruiseki.okcore.helper.ItemHelpers;
import shim.net.minecraft.text.Text;

@JFMUYPlugin(value = "emi")
@EmiEntrypoint
public class JemiPlugin implements IModPlugin, EmiPlugin {
    ...
    public static IJFMUYRuntime runtime;
    ...
    @Override
    public void registerSubtypes(ISubtypeRegistry registration) {
        ...
        registry.addGlobalGuiHandlers(new IGlobalGuiHandler() {
            ...
            public @Nonnull List<Rectangle> getGuiExtraAreas() {
                ...
                if (bookmarkButton.visible) {
                    areas.add(
                        new Rectangle(
                            bookmarkButton.xPosition,
                            bookmarkButton.yPosition,
                            bookmarkButton.width,
                            bookmarkButton.height));
                }
                return areas;
            }
        });
    ...
    @Override
    public void onRuntimeAvailable(IJFMUYRuntime runtime) {
    ...
    public static boolean isIngredientHidden(ITypedIngredient<?> typed) {
        try {
            ...
            return Internal.getIngredientFilter()
                .getBlacklist()
                .isIngredientBlacklisted(typed.ingredient(), helper);
        } catch (Throwable t) {
            return false;
        }
    }
    ...
    public void register(EmiRegistry registry) {
        ...
        Set<String> handledNamespaces = EmiAgnos.getPlugins()
            .stream()
        ...
        registry.addGenericExclusionArea((screen, consumer) -> {
            if (runtime != null && runtime.getIngredientListOverlay()
                ...
                Set<Rectangle> areas = runtime.getIngredientListOverlay()
                    .getGuiScreenHelper()
        ...
        registry.addGenericStackProvider((screen, x, y) -> {
            ...
            GuiScreenHelper helper = runtime.getIngredientListOverlay()
                .getGuiScreenHelper();
        ...
        List<IRecipeCategory> categories = Lists.newArrayList(
        ...
        for (IRecipeCategory c : categories) {
            ...
            try {
                ...
                List<EmiStack> catalysts = runtime.getRecipeRegistry()
                    .getRecipeCatalysts(c)
                    .stream()
                ...
                if (handledNamespaces.contains(id.getResourceDomain())) {
                    EmiLog.info("[JEMI] Skipping recipe category " + id + " because mod is already handled");
                    continue;
                }
                if (existingCategories.contains(id)) {
                    EmiLog.info(
                        "[JEMI] Skipping recipe category " + id + " because native EMI recipe category already exists");
                    continue;
                }
                ...
                List<IRecipeWrapper> recipes = runtime.getRecipeRegistry()
                    .getRecipeWrappers(c);
    ...
    private void addInfoRecipes(EmiRegistry registry, IRecipeCategory category) {
        ...
        for (IRecipeWrapper recipe : (List<IRecipeWrapper>) (List<?>) runtime.getRecipeRegistry()
            .getRecipeWrappers(category)) {
    ...
    // if (outputs.size() > 1) {
    // replacement = new EmiPatternCraftingRecipe(inputs, EmiStack.EMPTY, category.getRegistryName(recipe),
    // builder.shapeless) {
    //
    // @Override
    ...
    // }
    // }
    // registry.removeRecipes(r -> r instanceof EmiCraftingRecipe && replaced.contains(r.getId()) &&
    // !replacements.contains(r));
    // }
    ...
    private void parseSubtypes(EmiRegistry registry) {
        if (subtypeRegistry != null) {
            ...
            for (Item item : EmiPort.getItemRegistry()
                ...
                if (ItemHelpers.isEmpty(defaultStack)) continue;
                if (subtypeRegistry.hasSubtypeInterpreter(defaultStack)) {
                    registry.setDefaultComparison(
                        item,
                        Comparison
                            .compareData(stack -> { return subtypeRegistry.getSubtypeInfo(stack.getItemStack()); }));
                }
            }
    ...
    private static EmiRecipeHandler<?> getRecipeHandler(Container handler, EmiRecipe recipe) {
        ...
        if (category != null) {
            IRecipeTransferHandler transferHandler = runtime.getRecipeRegistry()
