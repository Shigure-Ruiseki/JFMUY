...
import ruiseki.jfmuy.plugins.jemi.impl.JemiIngredientAcceptor;
import ruiseki.jfmuy.plugins.jemi.impl.JemiIngredients;
import ruiseki.jfmuy.plugins.jemi.impl.JemiRecipeLayoutBuilder;
import ruiseki.jfmuy.plugins.jemi.impl.JemiRecipeSlot;
import ruiseki.jfmuy.plugins.jemi.impl.JemiRecipeSlotBuilder;
import ruiseki.jfmuy.plugins.jemi.widget.JemiSlotWidget;
import ruiseki.jfmuy.plugins.jemi.widget.JemiTankWidget;
...
public class JemiRecipe<T extends IRecipeWrapper> implements EmiRecipe {
    ...
    private String getContentId() {
        ...
        for (List<? extends EmiIngredient> ingredients : Arrays.asList(inputs, catalysts, outputs)) {
            for (EmiIngredient ingredient : ingredients) {
                sb.append('/');
                String ids = ingredient.getEmiStacks()
                    .stream()
                    .filter(s -> !s.isEmpty() && s.getId() != null)
                    .map(emiStack -> EmiUtil.subId(emiStack.getId()))
                    .map(
                        rl -> rl.replace(':', '.')
    ...
    public void addWidgets(WidgetHolder widgets) {
        ...
        Optional<IRecipeLayoutDrawable> opt = Optional
            .ofNullable(RecipeLayout.create(-1, category, recipe, null, posX, posY));
        if (opt.isPresent()) {
            ...
            for (JemiRecipeSlotBuilder sb : builder.slots) {
                ...
                slot.layout = opt.get();
                if ((slot.tankInfo != null && !slot.getIngredients(JemiUtil.getFluidType())
        ...
        // category.createRecipeExtras(extras, recipe,
    ...
    public class JemiWidget extends Widget {
        ...
        public List<TooltipComponent> getTooltip(int mouseX, int mouseY) {
            ...
            list.addAll(
                Stream.of(
                    Optional.ofNullable(category.getTooltipStrings(mouseX, mouseY))
                        .orElse(shim.java.List.of()),
                    Optional.ofNullable(recipe.getTooltipStrings(mouseX, mouseY))
                        .orElse(shim.java.List.of()))
                    .flatMap(List::stream)
                    .map(s -> TooltipComponent.of(EmiPort.literal(s)))
                    .collect(Collectors.toList()));
            return list;
        }
        ...
        // return recipe.handleInput(recipe, EmiScreenManager.lastMouseX, EmiScreenManager.lastMouseY,
