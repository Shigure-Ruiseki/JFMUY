...
public class JemiRecipeHandler<T extends Container> implements EmiRecipeHandler<T> {
    ...
    public JemiRecipeHandler(IRecipeTransferHandler handler) {
        ...
        /*
         * if (type != null) {
         * List<IRecipeCategory<R>> categories = (List<IRecipeCategory<R>>) (Object)
         * JemiPlugin.runtime.getRecipeManager().createRecipeCategoryLookup().includeHidden().limitTypes(List.of(type)).
         * get().toList();
    ...
    public boolean craft(EmiRecipe recipe, EmiCraftContext<T> context) {
        ...
        if (err == null || err.getType() == IRecipeTransferError.Type.USER_FACING) {
            Minecraft.getMinecraft()
                .displayGuiScreen(context.getScreen());
        }
        return err == null || err.getType() == IRecipeTransferError.Type.USER_FACING;
    }
    ...
    public void render(EmiRecipe recipe, EmiCraftContext<T> context, List<Widget> widgets, DrawContext raw) {
        ...
        if (err != null && view != null) {
            view.getRecipeSlots()
                .forEach(v -> v.highlight = 0);
            draw.push();
            draw.matrices()
                .translate(-100000, -100000, -100000);
            draw.matrices()
                .scale(0, 0, 0);
            err.showError(
                Minecraft.getMinecraft(),
    ...
    private IRecipeTransferError jeiCraft(EmiRecipe recipe, EmiCraftContext<T> context, boolean craft,
        ...
        try {
            ...
            return handler
                .transferRecipe(context.getScreenHandler(), view, client.thePlayer, context.getAmount() > 1, craft);
        } catch (Exception e) {
    ...
    private JemiRecipeLayoutBuilder createSlotsView(EmiRecipe recipe, List<Widget> widgets) {
        ...
        if (builder == null) {
            List<SlotWidget> slotWidgets = widgets.stream()
                .filter(w -> w instanceof SlotWidget)
                .map(w -> (SlotWidget) w)
            ...
            if (recipe instanceof EmiCraftingRecipe ecr) {
                if (ecr.shapeless) {
                    int inputSize = recipe.getInputs()
                        .size();
                    if (inputSize == 1) {
            ...
            if (recipe.getCategory() == VanillaEmiRecipeCategories.CRAFTING) {
                for (int i = recipe.getInputs()
                    .size() + blankedSlots; i < 9; i++) {
                    addIngredients(
                        builder,
                        slotWidgets,
                        Collections.singletonList(EmiStack.EMPTY),
                        RecipeIngredientRole.INPUT);
                }
            }
    ...
    private static void addIngredients(JemiRecipeLayoutBuilder builder, List<SlotWidget> widgets,
        List<? extends EmiIngredient> stacks, RecipeIngredientRole role) {
