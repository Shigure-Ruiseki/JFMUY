...
public class JemiScreenSwitch {
    ...
    private static boolean toJei(RecipeScreen rs) {
        ...
        if (value != null) {
            gui.show(
                JemiPlugin.runtime.getRecipeRegistry()
                    .createFocus(IFocus.Mode.OUTPUT, value));
            Optional.ofNullable(category)
                .ifPresent(
                    c -> gui.getRecipeGuiLogic()
                        .setRecipeCategory(c));
        } else {
    ...
    private static boolean toEmi(RecipesGui rg) {
        EmiStack stack = Optional.ofNullable(currentFocus(rg))
            .map(JemiUtil::getStack)
            .filter(s -> !s.isEmpty())
            .orElse(EmiStack.EMPTY);
        EmiRecipeCategory category = Optional.ofNullable(
            rg.getRecipeGuiLogic()
                .getSelectedRecipeCategory())
            .map(JemiScreenSwitch::getEmiCategory)
            .orElse(null);
        if (stack.isEmpty() && category == null) {
            return false;
        }
        rg.close();
        Optional.ofNullable(rg.getParentScreen())
            .filter(GuiContainer.class::isInstance)
            .map(GuiContainer.class::cast)
            .ifPresent(Minecraft.getMinecraft()::displayGuiScreen);
        open(stack, category).ifPresent(
            rs -> Optional.ofNullable(category)
                .ifPresent(rs::focusCategory));
        return true;
    }
    ...
    private static Object focusValue(RecipeScreen rs) {
        return Optional.ofNullable(currentRecipe(rs))
            ...
            .orElseGet(
                () -> Optional.of(rs.getHoveredStack())
                    .filter(h -> !h.isEmpty())
                    .map(
                        h -> JemiUtil.getTyped(
                            h.getEmiStacks()
    ...
    private static EmiRecipe currentRecipe(RecipeScreen rs) {
        ...
        try {
            List<WidgetGroup> currentPage = (List<WidgetGroup>) ReflectionHelper
                .findField(RecipeScreen.class, "currentPage")
                .get(rs);
    ...
    private static Object currentFocus(RecipesGui rg) {
        List<RecipeLayout> layouts = rg.getRecipeLayouts();
        if (layouts != null) {
            return layouts.stream()
                .map(RecipeLayout::getFocus)
                .filter(Objects::nonNull)
                .map(f -> (Object) f.getValue())
                .findFirst()
                .orElse(null);
        }
    ...
    public static EmiRecipeCategory getEmiCategory(IRecipeCategory<?> category) {
        return JemiPlugin.CATEGORY_MAP.entrySet()
            .stream()
