...
public class JemiIngredientAcceptor {
    ...
    public void coerceStacks(ITooltipCallback<?> tooltipCallback,
        Map<IIngredientType<?>, JemiRecipeSlot.IngredientRenderer<?>> renderers) {
        if (tooltipCallback == null && renderers == null) {
            return;
        }
        for (EmiStack stack : stacks) {
            ITypedIngredient typed = JemiUtil.getTyped(stack)
                .orElse(null);
            if (typed != null && (stack instanceof JemiStack<?> || stack.getKey() instanceof Fluid)) {
                ...
                if (renderers != null && renderers.containsKey(typed.type())) {
                    IIngredientRenderer<Object> renderer = (IIngredientRenderer) renderers.get(typed.type())
                        .renderer();
                    List<String> tooltip = renderer.getTooltip(
                        Minecraft.getMinecraft(),
                        typed.ingredient(),
                        ITooltipFlag.TooltipFlags.NORMAL.isAdvanced());
                    if (tooltip != null) {
                        base.addAll(tooltip);
                    }
                }
    ...
    private void addStack(EmiStack stack) {
        if (!stack.isEmpty()) {
            ...
            if (itemStack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                stacks.addAll(
                    EmiStack.ofPotentialTag(itemStack)
                        .getEmiStacks());
            } else {
                stacks.add(stack);
            }
