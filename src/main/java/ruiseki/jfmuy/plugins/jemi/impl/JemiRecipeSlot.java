...
import ruiseki.jfmuy.gui.ingredients.CycleTimer;
...
public class JemiRecipeSlot implements IGuiIngredient<Object> {
    ...
    public <T> Stream<T> getIngredients(IIngredientType<T> ingredientType) {
        return (Stream<T>) getAllIngredientsStream().filter(t -> t.type() == ingredientType)
            .map(t -> t.ingredient());
    }
    ...
    public List<Object> getAllIngredients() {
        return getAllIngredientsStream().map(t -> t.ingredient())
    ...
    public Object getDisplayedIngredient() {
        ...
        Optional<ITypedIngredient<?>> ing = JemiUtil.getTyped(
            stack.getEmiStacks()
                .get(0));
