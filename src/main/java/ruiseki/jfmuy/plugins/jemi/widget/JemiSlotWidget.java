package ruiseki.jfmuy.plugins.jemi.widget;
...
import ruiseki.jfmuy.plugins.jemi.JemiStack;
...
public class JemiSlotWidget extends SlotWidget {
    ...
    private ITypedIngredient<?> getIngredient() {
        if (slot.renderers != null) {
            Optional<ITypedIngredient<?>> opt = JemiUtil.getTyped(
                getStack().getEmiStacks()
                    .get(0));
            if (opt.isPresent()) {
                return opt.get();
            }
    ...
    private IIngredientRenderer<?> getRenderer() {
        ...
        if (typed != null) {
            if (slot.renderers.containsKey(typed.type())) {
                return slot.renderers.get(typed.type())
                    .renderer();
            }
        }
        return null;
    ...
    public void render(DrawContext raw, int mouseX, int mouseY, float delta) {
        if (slot.background != null) {
            slot.background.drawable()
                .draw(Minecraft.getMinecraft(), x + slot.background.xOff(), y + slot.background.yOff());
        }
    ...
    public void drawOverlay(DrawContext raw, int mouseX, int mouseY, float delta) {
        ...
        if (slot.overlay != null) {
            ...
            context.matrices()
                .translate(0, 0, 200);
            slot.overlay.drawable()
                .draw(Minecraft.getMinecraft(), slot.x + slot.overlay.xOff(), slot.y + slot.overlay.yOff());
            context.pop();
        }
    ...
    public static void addTooltip(List<TooltipComponent> list, JemiRecipeSlot slot, EmiIngredient stack,
        IIngredientRenderer<?> renderer) {
        if (renderer != null) {
            if (stack.getEmiStacks()
                .size() == 1
                && stack.getEmiStacks()
                    .get(0) instanceof JemiStack js) {
                js = js.copy();
        ...
        if (slot.tooltipCallback != null) {
            try {
                ...
                List<String> original = stack.getEmiStacks()
                    .get(0)
                    .getTooltipText()
                    .stream()
                    .map(Text::getString)
                    .collect(Collectors.toList());
                Set<String> toRemove = original.stream()
                    .collect(Collectors.toSet());
                event.addAll(original);
                ITypedIngredient<?> typed = JemiUtil.getTyped(
                    stack.getEmiStacks()
                        .get(0))
                    .orElse(null);
                if (typed != null) {
                    ((ITooltipCallback) slot.tooltipCallback)
                        .onTooltip(slot.slotIndex, slot.isInput(), typed.ingredient(), event);
                }
                int index = Math.min(list.size(), 1);
                if (!event.isEmpty()) {
                    List<TooltipComponent> additions = Lists.newArrayList();
                    for (String s : event) {
                        if (!toRemove.contains(s) && !JemiIngredientAcceptor.FLUID_END.matcher(s)
                            .find()) {
