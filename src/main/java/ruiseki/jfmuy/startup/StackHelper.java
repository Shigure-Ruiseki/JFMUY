...
public class StackHelper implements IStackHelper {
    ...
    private MatchingItem getMatchingItem(Map<Integer, ItemStack> stacks, Iterable<ItemStack> contains) {
        for (ItemStack requiredStack : contains) {
            ...
            int requiredCount = Math.max(1, requiredStack.stackSize);
            for (Map.Entry<Integer, ItemStack> entry : stacks.entrySet()) {
                ItemStack availableStack = entry.getValue();
                if (availableStack.stackSize >= requiredCount && isEquivalent(requiredStack, availableStack)) {
     ...
     */
    public <R> boolean containsSameStacks(Iterable<ItemStackMatchable<R>> stacks,
        Iterable<ItemStackMatchable<R>> contains) {
    ...
    @Nullable
    public <R, T> R containsStackMatchable(Iterable<ItemStackMatchable<R>> stacks,
    ...
    private void addSubtypesFromCreativeTabToList(List<ItemStack> subtypeList, Item item, final int stackSize,
        ...
        try {
            ...
            Log.get()
                .warn("Caught a crash while getting sub-items of {}", item, e);
        }
        ...
        for (ItemStack subItem : subItems) {
            ...
            if (subItem.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                String itemStackInfo = ErrorUtil.getItemStackInfo(subItem);
                Log.get()
                    .error("Found an subItem of {} with wildcard metadata: {}", item, itemStackInfo);
            } else {
    ...
    private void addSubtypesToList(List<ItemStack> subtypesList, Iterable stacks) {
        for (Object obj : stacks) {
            if (obj instanceof ItemStack) {
                ...
                Log.get()
                    .error("Unknown object found: {}", obj);
            } else {
    ...
    private void toItemStackList(UniqueItemStackListBuilder itemStackListBuilder, @Nullable Object input,
        ...
        if (input instanceof ItemStack) {
            ...
            Log.get()
                .error("Unknown object found: {}", input);
        }
    }

    private void toItemStackList(UniqueItemStackListBuilder itemStackListBuilder, @Nullable ItemStack itemStack,
        boolean expandSubtypes) {
        if (itemStack != null) {
            if (expandSubtypes && itemStack.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
            ...
            if (subtypeInfo != null) {
                ...
                if (mode == UidMode.FULL) {
                    ...
                    if (stack.hasTagCompound()) {
                        NBTTagCompound nbtTagCompound = (NBTTagCompound) stack.getTagCompound()
                            .copy();
    ...
    private static class MatchingIndexed implements Iterable<ItemStackMatchable<Integer>> {
        ...
        public Iterator<ItemStackMatchable<Integer>> iterator() {
            return new DelegateIterator<Map.Entry<Integer, ItemStack>, ItemStackMatchable<Integer>>(
                map.entrySet()
                    .iterator()) {
