 ...
 * Search can be used to retrieve the set of all the values that were put in the index with keys that contain a given
 * input.
 * <p>
 ...
 * (if any) that can be
 * composed by sequentially traversing all the edges (e1, e2, ...) starting from startNode such that (e1.label +
 * e2.label + ...) is equal
 * to the stringSuffix.
 ...
 * contained within the GST.
 * In addition to those Strings, there are a set of different strings that are implicitly contained within the GST, and
 * it is composed of
 * the strings built by concatenating e1.label + e2.label + ... + $end, where e1, e2, ... is a proper path and $end is
 * prefix of any of
...
public class GeneralizedSuffixTree implements ISearchTree {
     ...
     * @param value      the value to add to the index
     */
    private Pair<Node, String> update(final Node inputNode, final String stringPart, final char newChar,
        final String rest, final int value) {
        Node s = inputNode;
        ...
        while (!endpoint) {
            ...
            if (null != tempEdge) {
                ...
                // the tree can contain deeper nodes at this stage because different strings were added by previous
                // iterations.
