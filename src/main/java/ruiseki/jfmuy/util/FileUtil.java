...
public final class FileUtil {
    ...
    public static boolean writeFileSafely(final File file, final FileOperation fileOperation) {
        ...
        try {
            ...
            if (fileBackup.exists() && !fileBackup.delete()) {
                Log.get()
                    .error("Could not delete old backup file {}", fileBackup.getAbsoluteFile());
            }

            return true;
        } catch (IOException e) {
            Log.get()
                .error("Failed to save file {}.", fileNew, e);
            if (fileBackup.exists()) {
                ...
                Log.get()
                    .error("Failed to restore file from backup file {}", fileBackup.getAbsoluteFile());
            }
            return false;
        }
    }
    ...
    public static void readZipFileSafely(final File file, final String zipEntryName,
        final ZipInputFileOperation fileOperation) {
    ...
    public static boolean writeZipFileSafely(final File file, final String zipEntryName,
        final ZipOutputFileOperation fileOperation) {
