...
import ruiseki.jfmuy.IngredientInformation;
...
public class IngredientListElement<V> implements IIngredientListElement<V> {
    ...
    protected IngredientListElement(V ingredient, IIngredientHelper<V> ingredientHelper,
        IIngredientRenderer<V> ingredientRenderer) {
        ...
        String modName = modIdUtil.getModNameForModId(modId)
        ...
        this.tooltipString = IngredientInformation
            .getTooltipString(ingredient, ingredientRenderer, modId, modName, displayName);
        ...
        if (ingredient instanceof ItemStack) {
            ...
            for (int oreId : OreDictionary.getOreIDs(itemStack)) {
                String oreName = OreDictionary.getOreName(oreId)
                    .toLowerCase(Locale.ENGLISH);
                oreDictStringBuilder.append(oreName)
                    .append(' ');
            }
            ...
            for (CreativeTabs creativeTab : item.getCreativeTabs()) {
                if (creativeTab != null) {
                    String creativeTabName = I18n.format(creativeTab.getTranslatedTabLabel())
                        .toLowerCase();
                    creativeTabStringBuilder.append(creativeTabName)
                        .append(' ');
                }
            }
