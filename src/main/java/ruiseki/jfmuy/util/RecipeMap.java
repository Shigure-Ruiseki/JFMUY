...
public class RecipeMap {
    ...
    public <T extends IRecipeWrapper, V> ImmutableList<T> getRecipeWrappers(IRecipeCategory<T> recipeCategory,
        V ingredient) {
        IIngredientHelper<V> ingredientHelper = ingredientRegistry.getIngredientHelper(ingredient);
    ...
    public <T extends IRecipeWrapper, R> void addRecipe(R recipe, T recipeWrapper, IRecipeCategory<T> recipeCategory,
        Map<Class, List> ingredientsByType) {
    ...
    private <T extends IRecipeWrapper, V, R> void addRecipe(R recipe, T recipeWrapper,
        IRecipeCategory<T> recipeCategory, Class<V> ingredientClass, List<V> ingredients) {
        IIngredientHelper<V> ingredientHelper = ingredientRegistry.getIngredientHelper(ingredientClass);

        Map<String, List<Object>> recipesForType = recipeTable.row(recipeCategory);
        // noinspection unchecked
        Map<String, List<T>> recipesWrappersForType = (Map<String, List<T>>) (Object) recipeWrapperTable
            .row(recipeCategory);
    ...
    public <T extends IRecipeWrapper, R> void removeRecipe(R recipe, T recipeWrapper, IRecipeCategory<T> recipeCategory,
        Map<Class, List> ingredientsByType) {
        for (Map.Entry<Class, List> entry : ingredientsByType.entrySet()) {
            if (entry != null) {
    ...
    private <T extends IRecipeWrapper, V, R> void removeRecipe(R recipe, T recipeWrapper,
        IRecipeCategory<T> recipeCategory, Class<V> ingredientClass, List<V> ingredients) {
        IIngredientHelper<V> ingredientHelper = ingredientRegistry.getIngredientHelper(ingredientClass);
